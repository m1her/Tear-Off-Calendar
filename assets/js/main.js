...
function handleClick(e) {
  /* updated */
  if (!e.classList.contains("page") || deleteStatus) return;
  if (shakeStatus) return;
  deleteStatus = true;
  shakeStatus = true;
...
function renderPage() {
  ...
  calenderContainer.className =
    "d-flex flex-column calender-container px-5 pt-3 page";
  ...
  calenderContainer.addEventListener("mouseup", (event) => {
    ...
    if (mouseUp > lastPosition + 150) {
      ...
      if (shakeStatus) return;
      shakeStatus = true;
      animatedElement.style.animation = `shake ${
        shakeSpeed / 2
  ...
  dateContentGap2.className =
    "d-flex justify-content-center date-content gap-2";
  ...
  let holidayList = holidays.filter((holiday) =>
    holiday.date.includes(holidayMonth)
  );
  let i = 0;
...
function sendImageInfo(type, guid, text, x, y) {
  if (typeof TheApp === "undefined") return;
